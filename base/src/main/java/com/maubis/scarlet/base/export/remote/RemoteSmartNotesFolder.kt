...
import com.maubis.scarlet.base.support.utils.SmartNoteCache
...
class RemoteSmartNotesFolder(context: Context, val folder: File) {

  val internalCache = SmartNoteCache(context)
  ...
  fun onInsert(note: ExportableNote) {
    GlobalScope.launch {
      val files = FormatBuilder().getFormats(note.description()).filter { it.formatType == FormatType.SMART_NOTE }.map { it.text }.toSet()
