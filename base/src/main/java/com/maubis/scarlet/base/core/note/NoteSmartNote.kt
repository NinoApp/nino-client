...
interface SmartNoteLoadCallback {
  fun onSuccess()

  fun onError()
}

class NoteSmartNote(context: Context) {

  val rootFolder = File(context.filesDir, "smart_notes")
  ...
  fun getFile(noteUUID: String, imageFormat: Format): File {
    if (imageFormat.formatType != FormatType.SMART_NOTE) {
  ...
  fun deleteAllFiles(note: Note) {
    for (format in FormatBuilder().getFormats(note.description)) {
      if (format.formatType === FormatType.SMART_NOTE) {
  ...
  fun loadPersistentFileToImageView(image: ImageView, file: File, callback: SmartNoteLoadCallback? = null) {
    GlobalScope.launch {
      if (!file.exists()) {
  ...
  fun loadThumbnailFileToImageView(noteUUID: String, imageUuid: String,
                                   image: ImageView,
                                   callback: SmartNoteLoadCallback? = null) {
    GlobalScope.launch {
