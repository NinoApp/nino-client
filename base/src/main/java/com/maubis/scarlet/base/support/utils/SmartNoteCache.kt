...
const val SMART_NOTE_CACHE_SIZE = 1024 * 1024 * 10L

class SmartNoteCache(context: Context) {

  private val persistentFolder = File(context.filesDir, "smart_notes")
  private val thumbnailFolder = File(context.cacheDir, "sn_thumbnails")
  ...
  private fun performEviction() {
    if (thumbnailCacheSize.get() <= SMART_NOTE_CACHE_SIZE) {
      return
    }
    ...
    GlobalScope.async {
      ...
      while (thumbnailCacheSize.get() > SMART_NOTE_CACHE_SIZE * 0.9 && index < files.size) {
